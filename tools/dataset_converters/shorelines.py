...
def oversample(label_dir, seg_map_path, out_dir, data_type):
    ...
    for cls in present_classes:
        ...
        for contour in contours:
            ...
            # If you have multiple areas and want to select one, 
            # here you might use clustering or some strategy to decide.
            # For simplicity, let's take the bounding box of found indices.
            # top_left_x = np.min(class_indices[1])
            # top_left_y = np.min(class_indices[0])
            # bottom_right_x = np.max(class_indices[1])
            # bottom_right_y = np.max(class_indices[0])
            
            # # Center of mass for simplicity, can be more sophisticated approach
            # center_x = (top_left_x + bottom_right_x) // 2
            # center_y = (top_left_y + bottom_right_y) // 2
            
            # Assuming the image is large enough, cropping 3000x3000 images around the center.
            # You might need to adjust this logic depending on the actual size and desired overlap.
